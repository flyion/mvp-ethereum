...
contract MSC_Interface { //Using an interface to interact with the MSC
    function updateFromOracle(bytes32 _policyId, bytes32 _flightId, uint256 _actualArrDte, uint8 _fltStatus) public;
}

///=== CONTRACT CODE =====
contract Flyion_Chainlink is ChainlinkClient, usingFlyionBasics, usingOraclize { //inherits  Ownable, Authorizable
    ...
    event LogCallbackChainlink(bytes32 query, uint256 actualArrDte);
    event ChainlinkActionNeeded(bytes32 policyId, bytes32 queryId, uint256 returnValue);
    ...
    uint256 oraclePayment; // chainlink payment 
        ...
        onlyAuthorized
    {
        ...
        bytes32 _flightId = createFlightId(_flight, _departure); //using Flyion_Basics method
    ...
    //update of our internal mappings: Policy and Flights
        ...
        bytes32 _queryId = sendChainlinkRequestTo(chainlinkOracleAddr, req, oraclePayment * LINK);
    ...
    function updateMSC(bytes32 queryId, uint256 _result) internal {
        uint256 actualArrDte = _result;
...
//-- ADMIN functions
    ...
    function changeOraclePayment(uint256 _newPayment) public onlyOwner {
        require(_newPayment > 0, "Oracle payment must be greater than 0");
        oraclePayment = _newPayment;
    }
