...
contract ERC20_Interface {
    ...
    function allowance(address owner, address spender) public returns (uint256 allowance);
    //event Transfer(address indexed from, address indexed to, uint tokens);
}

contract FlyionOraclize_Interface { //Using an interface to interact with the Oraclize address (no need to recompile the whole thing)
...
contract MSC is Ownable, Authorizable { //remove Ownable? Check bytesize
        ...
        policyExists(_policyId)
    {
        ...
        if(_test == 0) {
            FlyionOraclize_Interface(oracleAddress).triggerOracle(_policyId, _fltNum, _depDte, _expectedArrDte, _updateDelayTime, address(this));
        }
    ...
    function processPayments(bytes32 _policyId) internal 
